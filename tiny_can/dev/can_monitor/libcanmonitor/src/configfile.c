 ...
 *
 *  add cfg_read_color, cfg_write_color, cfg_write_byte, cfg_write_word functions
 *  by Klaus Demlehner <klaus@mhs-elektronik.de>
 */
...
/*!
 \brief cfg_read_color() reads a GdkColor value from the cfg
 \param cfg (ConfigFile*) source of the data
 \param section (gchar *) section in the file
 \param key (gchar *) key we want the value for
 \param color (GdkColor *) value to return
 \returns TRUE on success, FALSE on no key found
 */
gboolean cfg_read_color(ConfigFile * cfg, gchar * section, gchar * key, GdkColor *color)
{
ConfigSection *sect;
ConfigLine *line;
gchar *str;
...
if (!(line = cfg_find_string(sect, key)))
	return FALSE;
str = g_strcompress(line->value);
vector = g_strsplit(str," ", 3);
if (g_strv_length(vector) >= 3)
  {
...
/*!
 \brief cfg_write_color() writes a GdkColor value to the cfg
 \param cfg (ConfigFile*) source of the data
 \param section (gchar *) section in the file
 \param key (gchar *) key we want to write the value for
 \param value (GdkColor *) value to write
 */
void cfg_write_color(ConfigFile * cfg, gchar * section, gchar * key, GdkColor * color)
{
ConfigSection *sect;
ConfigLine *line;
gchar * tmpbuf;
...
if ((line = cfg_find_string(sect, key)))
  ...
  cfg_create_string(sect, key, tmpbuf);
g_free(tmpbuf);
}
...
/*!
 \brief cfg_write_byte() writes a int value to the cfg
 \param cfg (ConfigFile*) source of the data
 \param section (gchar *) section in the file
 \param key (gchar *) key we want to write the value for
 \param value (gint) value to write
 */
void cfg_write_byte(ConfigFile * cfg, gchar * section, gchar * key, unsigned char value)
{
gchar *strvalue;

strvalue = g_strdup_printf("%u", value);
...
/*!
 ...
 \param value (gint) value to write
 */
void cfg_write_word(ConfigFile * cfg, gchar * section, gchar * key, unsigned short value)
{
gchar *strvalue;
