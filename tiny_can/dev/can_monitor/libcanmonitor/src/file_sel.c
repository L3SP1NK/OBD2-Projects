...
if (full_file_name)
  ...
  if (g_file_test(full_file_name, G_FILE_TEST_IS_REGULAR))
    gtk_file_chooser_select_filename(chooser, full_file_name);
  else
    {
    if (g_file_test(full_file_name, G_FILE_TEST_IS_DIR))
      gtk_file_chooser_set_current_folder(chooser, full_file_name);
    else
      {
