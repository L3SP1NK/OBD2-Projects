...
# Copyright (C) 2015 - 2023, Klaus Demlehner <klaus@mhs-elektronik.de>
# 
#
...
#   locations:
#   - read path from '/etc/tiny_can/api.path'
#   - .            (current directory)
#   - ./lib        (lib in current directory)
#   - /usr/local/lib
#   - /usr/lib
#   - /opt/tiny_can/can_api
#
# ---------------------------------------------------------------------- 
# ChangeLog: 
...
# 12.06.2019 V1.01 Add Items in "TCAN_ERROR_CODES"
# 01.03.2020 V2.00 Add CAN-FD support
# 15.05.2022 V2.10 Update option values from new API driver version, add timestamp support
# 04.05.2023 V2.20 Updates for newer Python3 version
#                  read driver path from '/etc/tiny_can/api.path' file 
# ---------------------------------------------------------------------- 
...
VERSION = \
"""
MhsTinyCanDriver V2.21, 04.05.2023 (LGPL)
Rene Maurer (rene@cumparsita.ch)
...
# *********************************** Linux *****************************************            
    ...
    if (sys.maxsize > 2**32): 
        os_api_def_path = r'/opt/tiny_can/can_api/x64';
    else:        
        os_api_def_path = r'/opt/tiny_can/can_api/x32';
    try:
        api_path_fd = open('/etc/tiny_can/api.path', 'r') 
        api_path = api_path_fd.readline()
        api_path_fd.close()
    except:
        api_path = None;                
    if api_path: 		 
        if (sys.maxsize > 2**32): 
            os_api_path = api_path + r'/x64';
        else:        
            os_api_path = api_path + r'/x32';
        sharedLibraryLocations = [os_api_path,
                                  api_path,
	                              os.curdir,
...
TCAN_ERROR_CODES = {-1:'Driver not initialized',
                    ...
                    -28:'Main Thread not run',
                    -29:'User allocated memory not found',
                    -30:'Main Thread run fault',
                    -31:'USB/COM Port setup error',
                    -32:'Hardware not connected',
                    -33:'Variable type not match'}
...
CAN_FEATURE_LOM          = 0x0001  # Silent Mode (LOM = Listen only Mode)
CAN_FEATURE_ARD          = 0x0002  # Automatic Retransmission disable
CAN_FEATURE_TX_ACK       = 0x0004  # TX ACK (Gesendete Nachrichten bestätigen)
CAN_FEATURE_ERROR_MSGS   = 0x0008  # Error Messages Support
CAN_FEATURE_FD_HARDWARE  = 0x0010  # CAN-FD Hardware
CAN_FEATURE_FIFO_OV_MODE = 0x0020  # FIFO OV Mode (Auto Clear, OV CAN Messages)
CAN_FEATURE_HW_TIMESTAMP = 0x8000  # Hardware Time Stamp
...
TCAN_LOG_FLAG_ERROR         = 0x00000020
TCAN_LOG_FLAG_WARN          = 0x00000040
TCAN_LOG_FLAG_ERR_MSG       = 0x00000080
TCAN_LOG_FLAG_OV_MSG        = 0x00000100
...
TCAN_Keys_CanInitDriver=['FdMode',              
                         'CanTxAckEnable',      
                         'CanErrorMsgsEnable',  
                         'AutoStopCan',         
                         'MainThreadPriority',  
                         ...
                         'CommTryCount',        
                         'TxAckBypass'] 
TCAN_Keys_CanDeviceOpen = ['MainThreadPriority',   
                           'TimeStampMode',        
                           'CanRxDFifoSize',       
                           'CanTxDFifoSize',       
                           # *** Port Open Variable
...
class TCANFlagBits(Structure):
    _fields_ = [('DLC',c_uint8,4),       # 4bit
                ('TxD',c_uint8,1),       # 1bit
                ('Error',c_uint8,1),     # 1bit
                ('RTR',c_uint8,1),       # 1bit
                ('EFF',c_uint8,1),       # 1bit
                ('Source',c_uint8),      # 8bit
                ('FilHit',c_uint8,1),    # 1bit
                ('OV',c_uint8,1),        # 1bit
                ('Reserved1',c_uint8,6), # 6bit
                ('Reserved2',c_uint8)   # 16bit
                ] # 32bits total    
    def __init__(self):
        self.DLC=0
        self.TxD=0
...
# TCANFlagBits
class TCanFdFlagBits(Structure):
    _fields_ = [('Source',c_uint8),      # 8bit
                ('Length',c_uint8),      # 8bit
                ('TxD',c_uint8,1),       # 1bit
                ('Error',c_uint8,1),     # 1bit
                ('RTR',c_uint8,1),       # 1bit
...
class TCanFdFlags(Union):
    _fields_ = [('FlagBits',TCanFdFlagBits),
                ('Uint32',c_uint32)] 
                        
# <*> Neu TCanFdMsg
class TCanFdMsg(Structure):
    _fields_ = [('Id', c_uint32),
                ('Flags', TCanFdFlags),
                ('Data', c_uint8 * 64),
                ('Sec', c_uint32),
...
class MhsTinyCanDriver:
    ...
    def __init__(self, dll=None, options=None, ex_mode=1, fd_mode=0):
        """
        Class Constructor
        ...
        self.ExModeEn = ex_mode
        self.FdModeEn = fd_mode
    ...
    def OpenComplete(self, index = None, options=None, snr=None, canSpeed=None, canDSpeed=None, canNBTR=None, canDBTR=None, listenOnly=0, txAck=None):
        """
        ...
        if canNBTR:            
            UpdateOptionDict(self.Options,{'CanSpeed1':0,'CanSpeed1User':canNBTR},self.logger)
        else:    
            if canSpeed:                    
                UpdateOptionDict(self.Options,{'CanSpeed1':canSpeed},self.logger)
        if canDBTR:                           
            UpdateOptionDict(self.Options,{'CanDSpeed1':0,'CanDSpeed1User':canDBTR},self.logger)
        else:    
            if canDSpeed:        
                UpdateOptionDict(self.Options,{'CanDSpeed1':canDSpeed},self.logger)
        if txAck:
            UpdateOptionDict(self.Options,{'CanTxAckEnable':txAck},self.logger)
    ...
    def initDriver(self, options=None):
        ...
        if self.ExModeEn == 1:
            if self.FdModeEn == 1:
                if OptionString:
                    err = self.CanExInitDriver("FdMode=1;" + OptionString)
                else:
                    err = self.CanExInitDriver("FdMode=1")
            else:
    ...
    def setCanBusSpeed(self, index=None, canSpeed=None, canDSpeed=None, canNBTR=None, canDBTR=None):
        """
    ...
    def FormatMessages(self, messages=None, showTimestamp=0):                  
        formatedMessages = []
        if messages:
            for message in messages:
                ...
                if showTimestamp == 3:         
                    formatedMessage = '{0:10} {1} {2:08x} {3} {4} {5}'.format(message.USec, dir, message.Id, frame_format, dlc, msg_data)
                else:           
                    if showTimestamp > 0:
                        formatedMessage = '{0:10}.{1:6} {2} {3:08x} {4} {5} {6}'.format(message.Sec, message.USec, dir, message.Id, frame_format, dlc, msg_data)
                    else:
        ...
        if messages:
            for message in messages:
                ...
                if message.Flags.FlagBits.TxD:
                    ...
                    dir = 'RX'                    
                len = message.Flags.FlagBits.Length;
                msg_data = ""
                if not message.Flags.FlagBits.RTR:
                    chars = 0;                                
                    for n in range(len):
                        if chars == 8:
                            ...
                            chars += 1
                        msg_data = msg_data + '%02X ' % message.Data[n]    
                formatedMessage = '{0} {1} {2} {3:08x} {4:2} {5}'.format(dir, fd_brs, frame_format, message.Id, len, msg_data)
                formatedMessages.append(formatedMessage)
        return formatedMessages        
    ...
    def TransmitData(self, index, msgId, msgData, msgLen=None, rtr = 0, eff = 0):
        ...
        @param eff: Extended Frame Format
        @return: Error Code (0 = No Error)    
    ...
    def TransmitFdData(self, index, msgId, msgData, msgLen=None, rtr = 0, eff = 0, fd = 1, brs = 0):
        """
        High Level Function to transmit a CAN Message
        @param index: Struct commonly used by the Tiny Can API, Drop the Index to select the FIFO      
        @param msgId: CAN ID of the Message
        ...
        for i in msgData:
            if type(i) != int:
                ...
                raise ValueError('List of integers expected but got {0} in list instead'.format(type(i)))
        if msgLen != None:
            Flags.FlagBits.Length = msgLen;
        else:
            l = len(msgData);
            if l > 64: 
                raise ValueError('Messages with more then 8 Bytes are not supported')
            else:                         
                Flags.FlagBits.Length = l          
        Flags.FlagBits.RTR = rtr
        Flags.FlagBits.EFF = eff
        Flags.FlagBits.FD = fd
        Flags.FlagBits.BRS = brs
        # Flags.FlagBits.TxD = 1 not necessary
        err = self.CanFdTransmit(index, msgId, msgData, flags=Flags.Uint32) 
        if err < 0:
            self.logger.error('CanFdTransmit Error-Code: {0}'.format(err))              
        return err       
    ...
    def CanInitDriver(self, options=None):
        ...
        if options is None: 
            err = self.so.CanInitDriver(c_char_p(None))
        else:
            err = self.so.CanInitDriver(c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanSetOptions(self, options = None):
        ...
        err = self.so.CanSetOptions(c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanDeviceOpen(self, index, options = None):
        ...
        if options is None:
            err = self.so.CanDeviceOpen(c_uint32(idx), c_char_p(None))
        else:    
            err = self.so.CanDeviceOpen(c_uint32(idx), c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanTransmit(self, index, msgId, msgData, flags):
        """
    ...
    def CanFdTransmit(self, index, msgId, msgData, flags):
        """
        API CALL - Transmit a CAN Message
        @param index: Struct commonly used by the Tiny Can API        
        @param msgId: CAN ID of the Message
        ...
        for i,b in enumerate(msgData):
            canMSG.Data[i] = b
        self.so.CanFdTransmit.restype = c_int32    
        err = self.so.CanFdTransmit(c_uint32(idx), pointer(canMSG), c_int(1)) # transmit once
        if err < 0:
            self.logger.error('CanTransmit Error-Code: {0}'.format(err))
        return err        
    ...
    def CanFdReceive(self, index=None, count=1):
        """
        API CALL - Read a CAN Message from FIFO or Buffer, depends on index
        @param index: Struct commonly used by the Tiny Can API
        @param count: Number of messages to be read 
        ...
        if num == 0:    
            ...
            if num < count:
                count = num;           
            TCanFdMsgArrayType = TCanFdMsg * count # Struct of multiple TCANMsg Instances without using Python List object
            TCanFdMsgArray = TCanFdMsgArrayType()
            self.so.CanFdReceive.restype = c_int32
            res = self.so.CanFdReceive(c_uint32(idx), pointer(TCanFdMsgArray), count)
        if res < 0:
            self.logger.info('CanFdReceive, Error-Code: {0}'.format(num))
            TCanFdMsgArray = None  # <*> Speicher freigeben ?          
        return res, TCanFdMsgArray    
...
#void CanExDataFree(void **data)
    ...
    def CanExCreateDevice(self, options = None):
        ...
        if options is None:
            err = self.so.CanExCreateDevice(byref(idx), c_char_p(None))
        else:
            err = self.so.CanExCreateDevice(byref(idx), c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanExInitDriver(self, options = None):
        ...
        if options is None:
            err = self.so.CanExInitDriver(c_char_p(None))
        else:
            err = self.so.CanExInitDriver(c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanExSetOptions(self, index, name, options):
        ...
        if options is None:
            err = self.so.CanExSetOptions(c_uint32(idx), c_char_p(None))
        else:    
            err = self.so.CanExSetOptions(c_uint32(idx), c_char_p(options.encode('utf-8')))
        if err < 0:
    ...
    def CanExSetAsByte(self, index, name, value):
        ...
        err = self.so.CanExSetAsByte(c_uint32(idx), c_char_p(name.encode('utf-8')), c_char(value))
        if err < 0:
    ...
    def CanExSetAsWord(self, index, name, value):
        ...
        err = self.so.CanExSetAsWord(c_uint32(idx), c_char_p(name.encode('utf-8')), c_uint8(value))
        if err < 0:
    ...
    def CanExSetAsLong(self, index, name, value):
        ...
        err = self.so.CanExSetAsLong(c_uint32(idx), c_char_p(name.encode('utf-8')), c_int32(value))
        if err < 0:
    ...
    def CanExSetAsUByte(self, index, name, value):
        ...
        err = self.so.CanExSetAsUByte(c_uint32(idx), c_char_p(name.encode('utf-8')), c_uint8(value))
        if err < 0:
    ...
    def CanExSetAsUWord(self, index, name, value):
        ...
        err = self.so.CanExSetAsUWord(c_uint32(idx), c_char_p(name.encode('utf-8')), c_uint16(value))
        if err < 0:
    ...
    def CanExSetAsULong(self, index, name, value):
        ...
        err = self.so.CanExSetAsULong(c_uint32(idx), c_char_p(name.encode('utf-8')), c_uint32(value))
        if err < 0:
    ...
    def CanExSetAsString(self, index, name, value = None):
        ...
        if value is None:        
            err = self.so.CanExSetAsString(c_uint32(idx), c_char_p(name.encode('utf-8')), c_char_p(None))
        else:
            err = self.so.CanExSetAsString(c_uint32(idx), c_char_p(name.encode('utf-8')), c_char_p(value.encode('utf-8')))
        if err < 0:
    ...
    def CanExGetAsByte(self, index, name):
        ...
        err = self.so.CanExGetAsByte(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsWord(self, index, name):
        ...
        err = self.so.CanExGetAsWord(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsLong(self, index, name):
        ...
        err = self.so.CanExGetAsLong(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsUByte(self, index, name):
        ...
        err = self.so.CanExGetAsUByte(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsUWord(self, index, name):
        ...
        err = self.so.CanExGetAsUWord(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsULong(self, index, name):
        ...
        err = self.so.CanExGetAsULong(c_uint32(idx), c_char_p(name.encode('utf-8')), byref(value))
        if err < 0:
    ...
    def CanExGetAsString(self, index, name):
        ...
        err = self.so.CanExGetAsString(c_uint32(idx), c_char_p(name.encode('utf-8')) , byref(str))
        if err < 0:
